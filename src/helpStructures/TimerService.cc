...
{
	if (timerIndex < 0)
		EV_ERROR << "cancelTimer(): timerIndex=" << timerIndex << "negative index is not allowed";
	if (timerIndex >= TIMER_MAX_SIZE)
		EV_ERROR << "cancelTimer(): timerIndex=" << timerIndex << " is too large";
...
{
	if (timerIndex < 0)
		EV_ERROR << ("setTimer(): timerIndex=%i negative index is not allowed",timerIndex);
	if (timerIndex >= TIMER_MAX_SIZE)
		EV_ERROR << ("setTimer(): timerIndex=%i is too large",timerIndex);
...
{
	if (timerIndex < 0)
		EV_ERROR << ("getTimer(): timerIndex=%i negative index is not allowed",timerIndex);
	if (timerIndex >= TIMER_MAX_SIZE)
		EV_ERROR << ("getTimer(): timerIndex=%i is too large",timerIndex);
	if (timerIndex >= timerMessages.size())
		return -1;
	if (timerMessages[timerIndex] == NULL)
		return -1;
