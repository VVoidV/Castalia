...
{
	if (index < 0)
	    EV_ERROR << "Negative output index not permitted";
					...
					double max, int buckets)
{
	if (min >= max || buckets < 1)
	    EV_ERROR << ("ERROR: declareHistogram failed, bad parameters");
...
void CastaliaModule::powerDrawn(double power)
{
    if (!classPointers.resourceManager) {
		...
		if (name.compare("Radio") == 0) 
		    ...
		    EV_ERROR << getFullPath().c_str() << "module has no rights to call drawPower() function";
        if (!classPointers.resourceManager) 
            EV_ERROR << ("Unable to find pointer to resource manager module");
    }
