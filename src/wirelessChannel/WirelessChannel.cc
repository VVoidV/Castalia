...
void WirelessChannel::initialize(int stage)
{
	...
	if (nodeLocation == NULL)
		EV_ERROR << ("Could not allocate array nodeLocation\n");

	cellOccupation = new list<int>[numOfSpaceCells];
	if (cellOccupation == NULL)
		EV_ERROR << ("Could not allocate array cellOccupation\n");

	cTopology *topo;	// temp variable to access initial location of the nodes
	topo = new cTopology("topo");
	topo->extractByNedTypeName(cStringTokenizer("castalia_3_2.src.node.Node").asVector());
	...
	for (int i = 0; i < numOfNodes; i++) {
		...
		if (!onlyStaticNodes) {
			...
			if (cell < 0 || cell >= numOfSpaceCells) {
				EV_ERROR << ("Cell out of bounds for node %i, please check your mobility module settings\n", i);
			}
	...
	if (pathLoss == NULL)
		EV_ERROR << ("Could not allocate array pathLoss\n");
	...
	if (nodesAffectedByTransmitter == NULL)
		EV_ERROR << ("Could not allocate array nodesAffectedByTransmitter\n");
...
void WirelessChannel::handleMessage(cMessage * msg)
{
	...
	case WC_NODE_MOVEMENT:{
	     ...
	     *****************************************************/
			...
			if (onlyStaticNodes)
				EV_ERROR << ("Error: Rerceived WS_NODE_MOVEMENT msg, while onlyStaticNodes is TRUE");
			...
			if ((nodeLocation[srcAddr].x < 0.0) ||
				...
				(nodeLocation[srcAddr].z < 0.0))
					EV_ERROR << ("Wireless channel received faulty WC_NODE_MOVEMENT msg. We cannot have negative node coordinates");
	...
	default:{
			EV_ERROR << ("ERROR: Wireless Channel received unknown message kind=%i", msg->getKind());
			break;
		}
	}
	delete msg;
