...
void CustomizablePhysicalProcess::handleMessage(cMessage * msg)
{
	if (msg->getKind() != PHYSICAL_PROCESS_SAMPLING)
		EV_ERROR << ("Physical Process received message other than PHYSICAL_PROCESS_SAMPLING");
...
void CustomizablePhysicalProcess::readIniFileParameters(void)
{
	...
	if (inputType == DIRECT_NODE_VALUES) {
		...
		if (numSources > 5)
			EV_ERROR << ("Physical Process parameter \"numSources\" has been initialized with invalid value \"%d\"",
numSources);
