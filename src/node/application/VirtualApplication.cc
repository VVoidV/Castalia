...
void VirtualApplication::initialize()
{
	...
	startup_delay += dblrand(0) * (double)parent->par("startupRandomization");
...
void VirtualApplication::handleMessage(cMessage * msg)
{
	...
	switch (msgKind) {
		...
		default:
		{
			EV_ERROR << ("Application module received unexpected message");
		}
	}
...
void VirtualApplication::toNetworkLayer(cMessage * msg)
{
	if (msg->getKind() == APPLICATION_PACKET)
		EV_ERROR << ("toNetworkLayer() function used incorrectly to send APPLICATION_PACKET without destination Network address");
