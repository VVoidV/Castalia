...
void VirtualMobilityManager::initialize()
{
	...
	if (!network)
		EV_ERROR << ("Unable to obtain SN pointer for deployment parameter");
		
	if (!wchannel)
		EV_ERROR << ("Unable to obtain wchannel pointer");
...
void VirtualMobilityManager::parseDeployment() {
	...
	while (ct != NULL) {
		...
		if (c[0] && c[0] == '[') {
			c++;
			if (!c[0] || c[0] < '0' || c[0] > '9')
				EV_ERROR << ("Bad syntax of SN.deployment parameter: expecing a digit at\n%s", c);
			start_range = strtol(c, &c, 10);
			if (!c[0] || (c[0] != ']' && c[0] != '.'))
				EV_ERROR << ("Bad syntax of SN.deployment parameter: expecing a ']' or '.' at\n%s", c);
			...
			if (!c[0] || c[0] != ']')
				EV_ERROR << ("Bad syntax of SN.deployment parameter: expecing a ']' at\n%s", c);
			c++;
			if (c[0] != '-' || !c[1] || c[1] != '>')
				EV_ERROR << ("Bad syntax of SN.deployment parameter: expecing a '->' at\n%s", c);
			c += 2;
		...
		if (c[0] < '0' || c[0] > '9')
			EV_ERROR << ("Bad syntax of SN.deployment parameter: expecing 'uniform', 'center', 'NxN[xN]' or 'randomized_NxN[xN]' at\n%s", c);
		gridx = strtol(c, &c, 10);
		if (c[0] != 'x' || !c[1] || c[1] < '0' || c[1] > '9')
			EV_ERROR << ("Bad syntax of SN.deployment parameter: expecing 'x' followed by a digit at\n%s", c);
		c++;
		gridy = strtol(c, &c, 10);
		if (c[0]) {
			if (c[0] != 'x' || !c[1] || c[1] < '0' || c[1] > '9') {
