...
void ResourceManager::initialize()
{
	...
	if (baselineNodePower < 0 || periodicEnergyCalculationInterval < 0)
		EV_ERROR << ("Illegal values for baselineNodePower and/or periodicEnergyCalculationInterval in resource manager module");
...
void ResourceManager::handleMessage(cMessage * msg)
{
	...
	switch (msg->getKind()) {
		...
		default:{
			EV_ERROR << ("ERROR: Unexpected message received by resource manager: %s", msg->getKind());
		}
	}
	delete msg;
}
