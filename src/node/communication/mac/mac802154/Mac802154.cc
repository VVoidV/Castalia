	...
	switch (index) {
		...
		case PERFORM_CCA:{
			...
			if (CCAcode == CLEAR) {
				...
				EV_ERROR << ("802.15.4 MAC internal error, isChannelClear() function called when radio is NOT READY");
			}
			break;
		}
...
void Mac802154::readIniFileParameters(void)
{
	...
	if (isPANCoordinator) {
		if (!isFFD) {
			EV_ERROR << ("Only full-function devices (isFFD=true) can be PAN coordinators");
		}
		...
		if (frameOrder < 0 || beaconOrder < 0 || beaconOrder > 14
			...
			EV_ERROR << ("Invalid combination of frameOrder and beaconOrder parameters. Must be 0 <= frameOrder <= beaconOrder <= 14");
		}
		...
		if (beaconInterval <= 0 || frameInterval <= 0) {
			EV_ERROR << ("Invalid parameter combination of baseSlotDuration and numSuperframeSlots");
		}
	}
}
...
{
	if (!nextPacket)
		EV_ERROR << ("MAC 802.15.4 internal error: collectPacketState called while nextPacket pointer is NULL");
	if (nextPacket->getMac802154PacketType() == MAC_802154_DATA_PACKET
