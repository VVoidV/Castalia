...
void Radio::completeStateTransition()
{
	...
	switch (state) {
		...
		default:{
			EV_ERROR << ("Internal Radio module error: bad state transition requested\n");
		}
	}
}
...
double Radio::SNR2BER(double SNR_dB)
{
	switch (RXmode->modulation) {
		...
		default:
			EV_ERROR << ("Modulation type not defined");
	}
	return 1.0;
}
...
int Radio::bitErrors(double BER, int numOfBits, int maxBitErrorsAllowed)
{
	...
	for (bitErrors = 0; bitErrors <= maxBitErrorsAllowed; bitErrors++) {
		...
		if (dblrand(0) <= prob / (1.0 - cumulativeProbabilityOfUnrealizedEvents))
			break;
...
void Radio::readIniFileParameters(void)
{
	...
	if (startingState.compare("RX") == 0)
		...
		EV_ERROR << ("Unknown basic state name '%s'\n", startingState.c_str());
...
void Radio::parseRadioParameterFile(const char *fileName)
{
	if (strlen(fileName) == 0)
		EV_ERROR << ("Radio parameters file not specified");
	ifstream f(fileName);
	if (!f.is_open())
		EV_ERROR << ("Error reading from radio parameters file %s\n", fileName);
	...
	while (getline(f, s)) {
		...
		if (s.compare("RX MODES") == 0) {
			...
			EV_ERROR << ("Bad syntax of radio parameters file, expecting label:\n%s", ct);
		else {
			if (section == 1) {
				...
				if (ct == NULL)
					EV_ERROR << ("Bad syntax of radio parameters file, expecting rx mode name:\n%s", ct);
				rxmode.name = string(ct);
				ct = t.nextToken();

				if (parseFloat(ct, &rxmode.datarate))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting data rate for rx mode %s:\n%s",
				...
				if (parseInt(ct, &rxmode.bitsPerSymbol))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting bits per symbol for rx mode %s:\n%s",
							rxmode.name.c_str(), ct);
				ct = t.nextToken();
				if (parseFloat(ct, &rxmode.bandwidth))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting bandwidth for rx mode %s:\n%s",
							rxmode.name.c_str(), ct);
				ct = t.nextToken();
				if (parseFloat(ct, &rxmode.noiseBandwidth))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting noise bandwidth for rx mode %s:\n%s",
							rxmode.name.c_str(), ct);
				rxmode.noiseBandwidth *= 1000.0;	//convert kHz to Hz
				ct = t.nextToken();
				if (parseFloat(ct, &rxmode.noiseFloor))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting noise floor for rx mode %s:\n%s",
							rxmode.name.c_str(), ct);
				ct = t.nextToken();
				if (parseFloat(ct, &rxmode.sensitivity))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting sensitivity for rx mode %s:\n%s",
							rxmode.name.c_str(), ct);
				ct = t.nextToken();
				if (parseFloat(ct, &rxmode.power))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting power for rx mode %s:\n%s",
							rxmode.name.c_str(), ct);
				ct = t.nextToken();
				if (ct != NULL)
					EV_ERROR << ("Bad syntax of radio parameters file, unexpected input for rx mode %s:\n%s",
							rxmode.name.c_str(), ct);

				list<RXmode_type>::iterator it1;
				for (it1 = RXmodeList.begin(); it1 != RXmodeList.end(); it1++) {
					if (rxmode.name.compare(it1->name) == 0)
						EV_ERROR << ("Bad syntax of radio parameters file, duplicate RX mode %s",
						     rxmode.name.c_str());
				}
				...
				ct = t.nextToken();
				if (ct == NULL)
					EV_ERROR << ("Bad syntax of radio parameters file, expecting Tx_dBm or Tx_mW label");
				...
				if (TxLevelList.empty()) {	// will insert new elements
					while ((ct = t.nextToken())) {
						...
						if (parseFloat(ct, &tmp))
							EV_ERROR << ("Bad syntax of radio parameters file, expecting power value for tx level:\n%s", ct);
						if (type == 1)
					...
					for (it1 = TxLevelList.begin(); it1 != TxLevelList.end(); it1++) {
						...
						double tmp;
						if (ct == NULL)
							EV_ERROR << ("Bad syntax of radio parameters file, expecting number of elements in Tx_dBm list and tx_mW list must be equal");
						if (parseFloat(ct, &tmp))
							EV_ERROR << ("Bad syntax of radio parameters file, expecting power value for tx level:\n%s", ct);
						if (type == 1)
				...
				if (parseFloat(ct, &sleeplvl.power))
					EV_ERROR << ("Bad syntax of radio parameters file, expecting power for sleep level %s:\n%s",
							sleeplvl.name.c_str(), ct);
				ct = t.nextToken();

				if (sleepLevelList.empty()) {	//this is the first element, to txUp should be '-'
					if (ct[0] != '-' || ct[1])
						EV_ERROR << ("Bad syntax of radio parameters file, expecting '-' as transition up delay for sleep level %s:\n%s",
								sleeplvl.name.c_str(), ct);
					ct = t.nextToken();
					if (ct[0] != '-' || ct[1])
						EV_ERROR << ("Bad syntax of radio parameters file, expecting '-' as transition up power for sleep level %s:\n%s",
								sleeplvl.name.c_str(), ct);
					...
					if (parseFloat(ct, &sleeplvl.transitionUp.delay))
						EV_ERROR << ("Bad syntax of radio parameters file, expecting transition up delay for sleep level %s:\n%s",
								sleeplvl.name.c_str(), ct);
					sleeplvl.transitionUp.delay /= 1000.0;	// convert msec to sec
					ct = t.nextToken();
					if (parseFloat(ct, &sleeplvl.transitionUp.power))
						EV_ERROR << ("Bad syntax of radio parameters file, expecting transition up power for sleep level %s:\n%s",
								sleeplvl.name.c_str(), ct);
				}
				...
				if (ct[0] == '-' && !ct[1]) {	// this is the last element
					...
					ct = t.nextToken();
					if (ct[0] != '-' || ct[1])
						EV_ERROR << ("Bad syntax of radio parameters file, expecting '-' as transition down power for sleep level %s:\n%s",
								sleeplvl.name.c_str(), ct);
					sleeplvl.transitionDown.delay = 0.0;
					sleeplvl.transitionDown.power = 0.0;
				} else {
					if (parseFloat(ct, &sleeplvl.transitionDown.delay))
						EV_ERROR << ("Bad syntax of radio parameters file, expecting transition down delay for sleep level %s:\n%s",
								sleeplvl.name.c_str(), ct);
					sleeplvl.transitionDown.delay /= 1000.0;	// convert msec to sec
					ct = t.nextToken();
					if (parseFloat(ct, &sleeplvl.transitionDown.power))
						EV_ERROR << ("Bad syntax of radio parameters file, expecting transition down power for sleep level %s:\n%s",
								sleeplvl.name.c_str(), ct);
				}

				ct = t.nextToken();
				if (ct != NULL)
					EV_ERROR << ("Bad syntax of radio parameters file, unexpected input for sleep level %s:\n%s",
							sleeplvl.name.c_str(), ct);
				...
				while (stateFrom < 3) {
					if (stateTo == stateFrom) {
						if (ct[0] != '-' || ct[1])
							EV_ERROR << ("Bad syntax of radio parameters file, expecting '-' as state transition value from %i to %i:\n%s",
									stateFrom, stateTo, ct);
					} else {
						double tmp;
						if (parseFloat(ct, &tmp))
							EV_ERROR << ("Bad syntax of radio parameters file, expecting state transition value from %i to %i:\n%s",
									stateFrom, stateTo, ct);
						if (section == 4)
				...
				if (ct != NULL)
					EV_ERROR << ("Bad syntax of radio parameters file, unexpected input for transition matrix:\n%s", ct);
			}
		}
	}
}
...
Modulation_type Radio::parseModulationType(const char *c)
{
	...
	if (modulation.compare("DIFFQPSK") == 0)
		return DIFFQPSK;
	EV_ERROR << ("Unknown modulation type parameter: '%s'\n", c);
	return CUSTOM;
}
...
Encoding_type Radio::parseEncodingType(const char *c)
{
	...
	if (encodingStr.compare("SECDEC"))
		return SECDEC;
	EV_ERROR << ("Unknown encoding type parameter: '%s'\n", c);
	return NRZ;
}
...
list<RXmode_type>::iterator Radio::parseRxMode(string modeName)
{
	...
	EV_ERROR << ("Unknown radio RX mode %s", modeName.c_str());
...
list<TxLevel_type>::iterator Radio::parseTxLevel(string txPower)
{
	...
	if (parseFloat(txPower.c_str(), &txPower_dBm))
		EV_ERROR << ("Unable to parse TxOutputPower %s", txPower.c_str());
...
list<TxLevel_type>::iterator Radio::parseTxLevel(double txPower)
{
	...
	EV_ERROR << ("Unknown Tx Output Power Level %f", txPower);
...
list<SleepLevel_type>::iterator Radio::parseSleepLevel(string sleepLevelName)
{
	...
	EV_ERROR << ("Unknown radio sleep level %s", sleepLevelName.c_str());
