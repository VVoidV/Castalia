...
void VirtualRouting::toMacLayer(cMessage * msg)
{
	if (msg->getKind() == NETWORK_LAYER_PACKET)
		EV_ERROR << ("toMacLayer() function used incorrectly to send NETWORK_LAYER_PACKET without destination MAC address");
...
void VirtualRouting::handleMessage(cMessage * msg)
{
	...
	switch (msgKind) {
		...
		default:{
			EV_ERROR << ("Network module recieved unexpected message: [%s]", msg->getName());
		}
	}
